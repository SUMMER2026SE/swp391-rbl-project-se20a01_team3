...
| UC28 | Chấp nhận / từ chối liên kết | 100% | Đủ SRS; SRS/API/test thống nhất trạng thái canonical `ACTIVE` (`active` trên API/DB). |
...
- Code/API/test dùng trạng thái PH-HS canonical `ACTIVE` (`active` trên API/DB); dữ liệu legacy `accepted` chỉ được migrate/đọc tương thích thành `active`.
