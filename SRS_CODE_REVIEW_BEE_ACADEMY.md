...
| UC28 - Chấp nhận / từ chối liên kết | 100% | HS xem lời mời, accept/reject, tự expire sau 7 ngày, audit log, thông báo PH, cho phép nhiều PH liên kết 1 HS. DB/API/test đã thống nhất `ACTIVE` canonical (`active` trên API/DB); `accepted` chỉ còn là giá trị legacy được migrate/đọc tương thích thành `active`. | Không còn lệch thuật ngữ trạng thái trong contract hiện tại. | Có flow consent dữ liệu nhạy cảm theo từng phụ huynh. |
...
1. Chuẩn hóa các điểm lệch SRS lớn còn lại: PayOS thay VNPay/MoMo, UC29 hủy một phía hay xác nhận hai phía.
